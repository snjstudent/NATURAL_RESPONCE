...
class AttentionEncoder(Encoder):
    def __init__(self, vocab_size, wordvec_size, hidden_size, dropout_ratio=0.35):
        ...
        self.params = self.embed.params + self.lstm.params + self.lstm_1.params + self.lstm_2.params +\
            self.dropout.params + self.dropout_1.params + \
            self.dropout_2.params + self.dropout_3.grads
        self.grads = self.embed.grads + self.lstm.grads + self.lstm_1.grads + self.lstm_2.grads +\
            self.dropout.grads + self.dropout_1.grads + \
            self.dropout_2.grads+self.dropout_3.grads
        self.hs = None

    def forward(self, xs):
        xs_1 = self.embed.forward(xs)
        xs = self.dropout.forward(xs_1)
...
class AttentionDecoder:
    def __init__(self, vocab_size, wordvec_size, hidden_size, dropout_ratio=0.35):
        ...
        self.dropout_2 = TimeDropout(dropout_ratio)
        self.dropout_3 = TimeDropout(dropout_ratio)
        layers = [self.embed, self.lstm, self.lstm_1, self.lstm_2, self.attention,
                  self.affine, self.dropout, self.dropout_1, self.dropout_2, self.dropout_3]
...
class AttentionSeq2seqss(Seq2seq):
